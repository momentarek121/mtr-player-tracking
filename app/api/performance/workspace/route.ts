...
export async function GET(req: NextRequest) {
  const coach = await getAuthenticatedPerformanceCoach(req);
  if (!coach) return NextResponse.json({ error: "جلسة المدرب غير صالحة أو لا تملك صلاحية الأداء البدني" }, { status: 401 });
...
export async function POST(req: NextRequest) {
  ...
  if (body.type === "assignment") {
    ...
    const { data, error } = await supabase.from("performance_coach_players").upsert({ coach_id: coachId, player_id: body.playerId, assigned_by: coachId, active: true }, { onConflict: "coach_id,player_id" }).select().single();
  ...
  if (body.type === "item") {
    ...
    if (!(await getManagedProgram(coachId, body.programId))) return NextResponse.json({ error: "البرنامج غير تابع للاعب مسند لهذا المدرب" }, { status: 403 });
  ...
  if (body.type === "media") {
    if (!body.itemId || !body.attachmentId) return NextResponse.json({ error: "itemId and attachmentId are required" }, { status: 400 });
    const { data: item } = await supabase.from("performance_program_items").select("id, program_id").eq("id", body.itemId).maybeSingle();
    if (!item || !(await getManagedProgram(coachId, item.program_id))) return NextResponse.json({ error: "التمرين غير تابع لهذا المدرب" }, { status: 403 });
    const { data, error } = await supabase.from("performance_item_media").insert({ item_id: body.itemId, attachment_id: body.attachmentId, timestamp_sec: body.timestampSec || null, note: body.note || null, created_by: coachId }).select().single();
    if (error) return NextResponse.json({ error: error.message }, { status: 500 });
    return NextResponse.json({ media: data }, { status: 201 });
  }
  if (!body.playerId || !body.title) return NextResponse.json({ error: "playerId and title are required" }, { status: 400 });
  if (!(await canManagePlayer(coachId, body.playerId))) return NextResponse.json({ error: "اللاعب غير مسند لهذا المدرب" }, { status: 403 });
  const { data, error } = await supabase.from("performance_programs").insert({ coach_id: coachId, player_id: body.playerId, title: body.title, goal: body.goal || null, start_date: body.startDate || null, end_date: body.endDate || null, notes: body.notes || null }).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
...
export async function PATCH(req: NextRequest) {
  ...
  if (body.type === "item") {
    ...
    if (!itemRow || !(await getManagedProgram(coachId, itemRow.program_id))) return NextResponse.json({ error: "التمرين غير تابع لهذا المدرب" }, { status: 403 });
  ...
  if (!body.programId || !(await getManagedProgram(coachId, body.programId))) return NextResponse.json({ error: "البرنامج غير تابع لهذا المدرب" }, { status: 403 });
  const update: Record<string, any> = {};
