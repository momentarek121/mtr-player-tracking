...
export default function PerformancePage() {
  ...
  const addExercise = async () => {
    ...
    setSaving(true);
    await fetch("/api/performance/workspace", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "item", programId: activeProgram.id, exerciseName: exerciseName.trim(), category: exerciseCategory, sets: exerciseSets, reps: exerciseReps, load: exerciseLoad }) });
  ...
  const toggleItem = async (item: any) => {
    await fetch("/api/performance/workspace", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ type: "item", itemId: item.id, completed: !item.completed }) });
    await load();
  };
