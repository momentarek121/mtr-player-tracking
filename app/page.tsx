...
function PlayerIntelligenceTab({ report, progress, notifications, onAcknowledge }: { report: any | null; progress: any | null; notifications: any[]; onAcknowledge: (notification: any) => void }) {
  ...
  const downloadReport = () => {
    ...
    const rows = [
      ...
      ["الالتزام بالتمارين", progress.adherencePercent ?? "—", comparison.previousAdherencePercent ?? "—", comparison.adherenceDelta ?? "—"],
      ["التمارين المكتملة", progress.completedExercises, comparison.previousCompletedExercises ?? "—", comparison.previousCompletedExercises == null ? "—" : progress.completedExercises - comparison.previousCompletedExercises],
      ["الحضور", progress.attendanceCount, comparison.previousAttendanceCount ?? "—", comparison.attendanceDelta ?? "—"],
      ["تغير الوزن كجم", progress.weightDeltaKg ?? "—", comparison.previousWeightDeltaKg ?? "—", progress.weightDeltaKg != null && comparison.previousWeightDeltaKg != null ? progress.weightDeltaKg - comparison.previousWeightDeltaKg : "—"],
      ["إشارات الشات", comparison.currentInsightCount ?? "—", comparison.previousInsightCount ?? "—", comparison.insightDelta ?? "—"],
      ["التعثر المحتمل", progress.blocker, "", ""],
      ["التوصية القادمة", progress.recommendation, "", ""],
    ];
    const csv = "\\ufeff" + rows.map((row) => row.map((cell) => `"${String(cell).replaceAll('"', '""')}"`).join(",")).join("\\n");
    const url = URL.createObjectURL(new Blob([csv], { type: "text/csv;charset=utf-8" }));
    const a = document.createElement("a"); a.href = url; a.download = "mtr-player-progress-report.csv"; a.click(); URL.revokeObjectURL(url);
  };
  return (
    <div className="space-y-4">
      ...
      {progress && <div className="bg-neutral-950 border border-teal-700/40 rounded-xl p-5"><div className="flex items-start justify-between gap-3 mb-4"><div><div className="text-sm font-semibold text-neutral-200">تقرير التنفيذ الفعلي — آخر 7 أيام</div><div className="text-[11px] text-neutral-500 mt-1">مقارنة بين الخطة وما تم تنفيذه فعلًا</div></div><div className="flex items-center gap-2"><button onClick={downloadReport} className="text-[10px] text-teal-300 border border-teal-700/40 rounded-full px-2.5 py-1 hover:bg-teal-400/10">تصدير CSV</button><button onClick={() => window.print()} className="text-[10px] text-neutral-400 border border-neutral-700 rounded-full px-2.5 py-1 hover:bg-neutral-800">طباعة</button><div className="text-[10px] text-teal-300 bg-teal-400/10 rounded-full px-2.5 py-1">UPDATED</div></div></div><div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4"><div className="bg-neutral-900 rounded-lg p-3"><div className="text-[11px] text-neutral-500">الالتزام</div><div className="text-lg font-semibold text-teal-300">{progress.adherencePercent === null ? "—" : `${progress.adherencePercent}%`}</div></div><div className="bg-neutral-900 rounded-lg p-3"><div className="text-[11px] text-neutral-500">التمارين</div><div className="text-lg font-semibold">{progress.completedExercises}/{progress.exerciseCount}</div></div><div className="bg-neutral-900 rounded-lg p-3"><div className="text-[11px] text-neutral-500">الحضور</div><div className="text-lg font-semibold">{progress.attendanceCount}</div></div><div className="bg-neutral-900 rounded-lg p-3"><div className="text-[11px] text-neutral-500">تغير الوزن</div><div className="text-lg font-semibold">{progress.weightDeltaKg === null ? "—" : `${progress.weightDeltaKg > 0 ? "+" : ""}${progress.weightDeltaKg} كجم`}</div></div></div><div className="grid md:grid-cols-2 gap-3"><div className="bg-orange-400/5 border border-orange-700/30 rounded-lg p-3"><div className="text-[11px] text-orange-300 mb-1">التعثر المحتمل</div><div className="text-xs text-neutral-300 leading-relaxed">{progress.blocker}</div></div><div className="bg-violet-400/5 border border-violet-700/30 rounded-lg p-3"><div className="text-[11px] text-violet-300 mb-1">توصية الأسبوع القادم</div><div className="text-xs text-neutral-300 leading-relaxed">{progress.recommendation}</div></div></div></div>}

      {progress?.comparison && <div className="bg-neutral-950 border border-sky-700/40 rounded-xl p-5"><div className="text-sm font-semibold text-neutral-200 mb-3">مقارنة الأسبوع الحالي بالسابق</div><div className="grid grid-cols-2 md:grid-cols-4 gap-2"><div className="bg-neutral-900 rounded-lg p-3"><div className="text-[11px] text-neutral-500">تغير الالتزام</div><div className={`text-lg font-semibold ${comparison.adherenceDelta > 0 ? "text-teal-300" : comparison.adherenceDelta < 0 ? "text-red-300" : "text-neutral-300"}`}>{comparison.adherenceDelta == null ? "—" : `${comparison.adherenceDelta > 0 ? "+" : ""}${comparison.adherenceDelta}%`}</div></div><div className="bg-neutral-900 rounded-lg p-3"><div className="text-[11px] text-neutral-500">تغير الحضور</div><div className="text-lg font-semibold">{comparison.attendanceDelta > 0 ? "+" : ""}{comparison.attendanceDelta ?? "—"}</div></div><div className="bg-neutral-900 rounded-lg p-3"><div className="text-[11px] text-neutral-500">تغير إشارات الشات</div><div className="text-lg font-semibold">{comparison.insightDelta > 0 ? "+" : ""}{comparison.insightDelta ?? "—"}</div></div><div className="bg-neutral-900 rounded-lg p-3"><div className="text-[11px] text-neutral-500">وزن الأسبوع السابق</div><div className="text-lg font-semibold">{comparison.previousWeightDeltaKg == null ? "—" : `${comparison.previousWeightDeltaKg > 0 ? "+" : ""}${comparison.previousWeightDeltaKg} كجم`}</div></div></div></div>}
